...
import { CommonGetFilters, GetSearchOutlinksResponse } from 'modules/Common/interfaces';
...
const getOutlinks =
  (filter: CommonGetFilters) => async (res: NextApiResponse<GetSearchOutlinksResponse>) => {
    try {
      ...
      const outlinks = await SearchManager.getOutlinks({
      ...
      res.json({ status: 'ok', message: 'Search outlinks details', outlinks });
      return res;
    } catch (e: any) {
...
const search = async (req: NextApiRequest, res: NextApiResponse) => {
  if (req.method === 'GET' && req.query.name) {
    return getOutlinks(req.query as any)(res);
  }
