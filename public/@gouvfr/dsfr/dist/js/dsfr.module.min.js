  ...
  mutate(mutations) {
    mutations.forEach((mutation) => {
      if (
        ...
        mutation.attributeName === this.disclosure.attributeName
      )
        this.disclosure.change(this.disclosed);
    });
  }
...
class Disclosure extends Instance {
  constructor(element) {
    ...
    const buttons = document.querySelectorAll(
      this.type.ariaControls
        ? `[aria-controls="${this.id}"]`
        : ns.attr.selector('controls', this.id)
    );
...
class KeyListener {
  ...
  _add(type, action) {
    if (this.collections[type] === undefined)
      this.collections[type] = new KeyActionCollection(type, this.element);
...
class Collapse extends Disclosure {
  ...
  static get type() {
    return DISCLOSURE_TYPES.expand;
  }
  static get selector() {
...
const BREADCRUMB_COLLAPSE_SELECTOR = `${api.core.ns.selector('breadcrumb')} ${api.core.ns.selector(
  'collapse'
)}`;
...
const build$7 = () => {
  ...
  for (let i = 0; i < breadcrumbNodes.length; i++)
    breadcrumbs.push(new Breadcrumb(breadcrumbNodes[i]));
};
  ...
  get focusables() {
    ...
    if (radios.length) {
      ...
      unordereds = unordereds.filter((unordered) => {
        if (
          unordered.tagName.toLowerCase() !== 'input' ||
          unordered.getAttribute('type').toLowerCase() !== 'radio'
        )
          return true;
    ...
    return concateneds.filter(
      (element) => element.tabIndex !== '-1' && isFocusable(element, this.element)
    );
  }
...
class Modal extends api.core.Disclosure {
  ...
  static get type() {
    return api.core.DISCLOSURE_TYPES.opened;
  }
  static get selector() {
    return MODAL_CLASS;
  }
...
class Table {
  ...
  getScrollPosition(side) {
    ...
    switch (side) {
      ...
      case RIGHT:
        return (
          this.tableContent.offsetWidth -
          this.tableElem.offsetWidth -
          this.tableElem.scrollLeft * inverter
        );
      default:
        return false;
    }
...
class TabsGroup extends api.core.DisclosuresGroup {
  ...
  add(tab) {
    ...
    if (this.length === 1 || tab.disclosed) this.current = tab;
      ...
      if (this._index > -1 && this._index !== index)
        tab.translate(index < this._index ? -1 : 1, true);
    }
  }
...
class Tab extends api.core.Disclosure {
  static get type() {
    return api.core.DISCLOSURE_TYPES.select;
  }
  static get selector() {
    return PANEL_CLASS;
  }
...
const HEADER_LINKS_GROUP_SELECTOR = `${HEADER_TOOLS_LINKS_SELECTOR} ${api.core.ns.selector(
  'links-group'
)}`;
